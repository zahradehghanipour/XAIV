...
class PreparedBufferExit(Exception):
    """Signal that we should stop verification early (prepared buffer path)."""
...
class ABCROWN:
    ...
    def incomplete_verifier(
        ...
        c_transposed = False
        tighten_input_bounds = arguments.Config["solver"]["invprop"][
        ...
        if len(specs) > 1:
            ...
            c = (
                ...
                .unsqueeze(1)
                .to(data)
            )  # c shape: (batch, 1, num_outputs)
            do_transpose = not arguments.Config["solver"][
                "optimize_disjuncts_separately"
            ]
            rhs = torch.tensor(np.array([item[1] for item in specs])).to(
                data
            )  # (batch, 1)
            ...
            rhs = torch.tensor(
                specs[0][1], dtype=data.dtype, device=data.device
            ).unsqueeze(0)
        ...
        if len(apply_output_constraints_to) > 0:
            ...
            for node in model.net.nodes():
                if node.are_output_constraints_activated_for_layer(
                    apply_output_constraints_to
                ):
        ...
        if torch.any((global_lb - rhs) > 0, dim=-1).all():
            ...
            print("[ZRL] verified with init bound!")
        ...
        if c_transposed:
            ret["lower_bounds"][model.final_name] = ret["lower_bounds"][
                model.final_name
            ].t()
            ret["upper_bounds"][model.final_name] = ret["upper_bounds"][
                model.final_name
            ].t()
    ...
    def bab(
        ...
        if arguments.Config["bab"]["branching"]["input_split"]["enable"]:
            ...
            result = general_bab(
                ...
                self.domain,
                x,
                refined_lower_bounds=lower_bounds,
                refined_upper_bounds=upper_bounds,
    ...
    def building_model_for_prepared_data(
        self,
        data_lb,
        data_ub,
        c,
    ...
    def complete_verifier(
        ...
        enable_incomplete = arguments.Config["general"][
        ...
        if enable_incomplete:
            ...
            (batched_vnnlib, init_global_lb, init_global_ub, lA, attack_images) = (
                sort_targets(
                    batched_vnnlib,
                    init_global_lb,
                    init_global_ub,
        ...
        # ZD,  A try and catch for early stop in the process of training and testing of the prepared data
        try:
            time_stamp = 0
            for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
                ...
                print(
                    f"Verifying property {property_idx} with {len(properties[0])} instances."
                )
                if (
                    arguments.Config["bab"]["cut"]["enabled"]
                    and arguments.Config["bab"]["initial_max_domains"] == 1
                ):
                    if init_global_lb[property_idx][0] > 0:
                ...
                if isinstance(properties[0][0], dict):
                    ...
                    x_range = torch.tensor(
                        properties[0], dtype=torch.get_default_dtype()
                    )
                    data_min = x_range.select(-1, 0).reshape(vnnlib_shape)
                    data_max = x_range.select(-1, 1).reshape(vnnlib_shape)
                    x = x_range.mean(-1).reshape(
                        vnnlib_shape
                    )  # only the shape of x is important.
                    data_dict = None
                if "tightened_input_bounds" in results:
                    assert (
                        results["tightened_input_bounds"][0][
                            property_idx : property_idx + 1
                        ].shape
                        == data_min.shape
                    )
                    data_min = results["tightened_input_bounds"][0][
                        property_idx : property_idx + 1
                    ]
                    data_max = results["tightened_input_bounds"][1][
                        property_idx : property_idx + 1
                    ]

                target_label_arrays = list(
                    properties[1]
                )  # properties[1]: (c, rhs, y, pidx)
                ...
                if data_min.shape[0] > 1:
                    l1_err_data_min = torch.norm(
                        (data_min[1:] - data_min[0:1]).view(-1), p=1
                    )
                    l1_err_data_max = torch.norm(
                        (data_max[1:] - data_max[0:1]).view(-1), p=1
                    )
                ...
                time_stamp += 1
                input_split = arguments.Config["bab"]["branching"]["input_split"][
                    "enable"
                ]
                init_failure_idx = np.array([])

                # ZD, checking if there is any prepared data (buffer) available so no need for colleting data for expert mode?
                prepared_buffer = arguments.Config["bab"]["branching"]["rl"][
                    "training"
                ]["preparedBuffer"]
                policy_path = arguments.Config["bab"]["branching"]["rl"]["training"][
                    "policynet"
                ]
                exp_dir = os.path.dirname(policy_path)

                if not prepared_buffer == "False":
                    print(f"\n[ZRL] === Expert mode === prepared_buffer is available!")
                    net = self.building_model_for_prepared_data(
                        data=x,
                        targets=init_failure_idx,
                        time_stamp=time_stamp,
                        data_ub=data_max,
                        data_lb=data_min,
                        data_dict=data_dict,
                        lower_bounds=trimmed_lower_bounds,
                        upper_bounds=trimmed_upper_bounds,
                        c=c,
                        reference_alphas=reference_alphas,
                        cplex_processes=cplex_processes,
                        activation_opt_params=results.get(
                            "activation_opt_params", None
                        ),
                        refined_betas=results.get("refined_betas", None),
                        rhs=rhs[0:1],
                        reference_lA=lA_trim,
                    ...
                    # ZD, this is for imitation using the script
                    script_training = arguments.Config["bab"]["branching"]["rl"][
                        "training"
                    ]["scriptTraining"]
                    if script_training:
                        return net

                    # ZD, this is for the time that we are following the pipeline and we have a prepared data
                    branching_heuristic = get_branching_heuristic(net)

                    # Loading and spliting the buffer to test and train based on step
                    seed = arguments.Config["general"]["seed"]
                    n_steps = arguments.Config["bab"]["branching"]["rl"]["training"][
                        "maxInstances"
                    ]

                    print(f"[ZRL]Loading buffer from: {prepared_buffer}")
                    buffer = branching_heuristic._buffer.restore(prepared_buffer)

                    train_buffer, test_buffer = (
                        branching_heuristic._buffer.split_by_steps(
                            f"{exp_dir}/prepared_buffer",
                            buffer,
                            0.8,
                            seed,
                            n_steps,
                    ...
                    epochs = arguments.Config["bab"]["branching"]["rl"]["expert"][
                        "learnToImitateExpertEpochs"
                    ]

                    branching_heuristic.imitate_from_prepared_data(
                        train_buffer, test_buffer, policy_path, epochs=epochs
                    )
                    return net

                # ZD, are we going to test the prepared data?
                testing_prepared_buffer = arguments.Config["bab"]["branching"]["rl"][
                    "training"
                ]["TestingPreparedBuffer"]
                testing_prepared_buffer = (
                    False
                    if testing_prepared_buffer == "False"
                    else testing_prepared_buffer
                )
                rl_eval = arguments.Config["bab"]["branching"]["rl"]["eval"]

                if testing_prepared_buffer and rl_eval:
                    print(f"[ZRL] === Testing mode on prepared_buffer ===")
                    net = self.building_model_for_prepared_data(
                        data=x,
                        targets=init_failure_idx,
                        time_stamp=time_stamp,
                    ...
                    branching_heuristic = get_branching_heuristic(net)
                    exp_dir = os.path.dirname(policy_path)
                    # ZD, loading the test buffer
                    test_buffer = branching_heuristic._buffer.restore(
                        path=f"{exp_dir}/prepared_buffer_test.pkl"
                    )
                    print("[ZRL]Test buffer loaded")

                    acc = branching_heuristic.evaluate_imitation_for_prepared_data(
                        test_buffer, policy_path
                    )
                    print(f"[ZRL]Accuracy of testing prepared data: {acc}")

                    # ZD, saving the result
                    txt_path = os.path.join(exp_dir, f"result.txt")
                    with open(txt_path, "a") as f:
                        print(f"[ZRL]Accuracy of testing prepared data: {acc}")
                        f.write(f"\n[ZRL]Accuracy of testing prepared data: {acc}\n")

                    raise PreparedBufferExit("prepared-buffer")

                if enable_incomplete and not input_split:
                    if (
                        len(init_global_lb) > 1
                    ):  # if batch == 1, there is no need to filter here.
                        ...
                        if len(init_failure_idx) == 0:
                            ...
                            continue
                        print(
                            f"Remaining spec index {init_failure_idx} with "
                        ...
                        (
                            ...
                            data_min,
                            data_max,
                            trimmed_lower_bounds,
                            trimmed_upper_bounds,
                            c,
                        ) = prune_by_idx(
                            reference_alphas,
                            init_verified_cond,
                    ...
                    bab_ret.append(
                        [index, l, nodes, time.time() - start_time_bab, pidx]
                    )

                # terminate the corresponding cut inquiry process if exists
                if cplex_cuts:
                    solved_c_rows.append(c)
                    terminate_mip_processes_by_c_matching(
                        cplex_processes, solved_c_rows
                    )
                ...
                # MP, added this for a specific heuristic for the preprocess input 
                # splitting.
                elif ret == "threshold":
                    return "threshold"
                elif ret == "timeout":
                    return "timeout"
                # ZD, TODO: This is probebly the place that causes the problem.
                # ZD, I have changed this be carefull!
                ...
                return "safe"
        except PreparedBufferExit as e:
            print(f"[ZRL]Early exit requested: {e}")
            return net
    ...
    def main(self, interm_bounds=None):
        ...
        if bab_args["backing_up_max_domain"] is None:
            arguments.Config["bab"]["backing_up_max_domain"] = bab_args[
                "initial_max_domains"
            ]
        (
            run_mode,
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            print(
                f"\n[ZRL] {'%' * 35} idx: {new_idx}, vnnlib ID: {vnnlib_id} {'%' * 35}"
            )
            if arguments.Config["general"]["save_output"]:
            ...
            if run_mode != "customized_data":
                if len(csv_item) == 3:
                    ...
                    arguments.Config["model"]["onnx_path"] = os.path.join(
                        file_root, csv_item[0]
                    )
                ...
                vnnlib = vnnlib_all[
                    new_idx
                ]  # vnnlib_all is a list of all standard vnnlib
            ...
            if arguments.Config["general"]["complete_verifier"].startswith(
                "Customized"
            ):
            ...
            if arguments.Config["attack"]["pgd_order"] == "before":
                (
                    ...
                    attack_margins,
                    all_adv_candidates,
                ) = self.attack(model_ori, x, vnnlib, verified_status, verified_success)
            else:
            ...
            if (
                ...
                if arguments.Config["general"]["complete_verifier"] == "auto":
                    ...
                    if complete_verifier in ["bab-refine", "mip"]:
                        ...
                        arguments.Config["bab"]["interm_transfer"] = (
                            interm_transfer_init
                        )
            ...
            if not verified_success and complete_verifier in ["bab-refine", "mip"]:
                ...
                verified_status, ret_mip = mip(
                    ...
                    vnnlib=vnnlib,
                    pgd_attack_example=[attack_image, attack_margins],
            ...
            if (
                ...
                batched_vnnlib = batch_vnnlib(
                    vnnlib
                )  # [x, [(c, rhs, y, pidx)]] in batch-wise
                ...
                verified_status = self.complete_verifier(
                    ...
                    - (time.time() - self.logger.start_time),
            ...
            prepared_buffer = arguments.Config["bab"]["branching"]["rl"]["training"][
                "preparedBuffer"
            ]
            if not prepared_buffer == "False":
                print("[ZRL] Imitation using preperd buffer has finished!")
                return verified_status  # this will returns the net

            elif testing_prepared_buffer:
                print("[ZRL] Testing preperd buffer has finished!")
                return verified_status  # this will returns the net
        ...
        if self.logger.run_mode == "single_vnnlib":
            ...
            if len(self.logger.bab_ret) != 0:
                return (
                    ...
                    self.logger.bab_ret[0][2],  # branching decisions
                    self.logger.bab_ret[0][3],  # bab_time
                    time.time() - self.logger.start_time, # all_time
                    arguments.Globals["out"]["initial_unstable_neurons"],
                    # arguments.Globals["out"]["stable_ratio"]
            ...
            return {
                "summary": self.logger.verification_summary,
                "statuses": self.logger.status_per_sample_list,
                "bab_ret": self.logger.bab_ret,
                "times": self.logger.time_all_instances,
            }
