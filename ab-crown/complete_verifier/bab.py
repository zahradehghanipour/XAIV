...
from auto_LiRPA.utils import AutoBatchSize, multi_spec_keep_func_all, stop_criterion_batch_any
from beta_CROWN_solver import LiRPANet
from branching_domains import BatchedDomainList, ShallowFirstBatchedDomainList, check_worst_domain

from cuts.cut_utils import biccos_update_general_beta, clean_net_mps_process, cplex_update_general_beta, fetch_cut_from_cplex
...
from input_split.input_split_on_relu_domains import InputReluSplitter, input_split_on_relu_domains

from lp_mip_solver import batch_verification_all_node_split_LP
from prune import prune_alphas
from torch_geometric.data import Data
from utils import Stats, check_auto_enlarge_batch_size, get_unstable_neurons, print_average_branching_neurons, print_splitting_decisions
...
def split_domain(
    ...
    min_batch_size = min(
        solver_args["min_batch_size_ratio"] * solver_args["batch_size"], batch
    )
    ...
    # [OVAL-GNN]
    # if LP in oval-gnn heuristics shows it is infeasible..  I guess we are running two
    # solvers  along side each other, so one might beat the other.
    if branching_decision is None:
        return torch.inf, torch.inf

    # print("[ZRL] [split_domain] Branching decision is", branching_decision)
    ...
    # ZD, next(iter(d['mask'].values())) grabs mask value for each layer
    # if branching_decision is shorter than that list of possible split nodes,
    # it means some nodes remain unsplit.
    # So, if this is false, then all nodes are already split and we are good to go.
    # e.g. batch = 4, so we have 4 desicion and just 3 node was remain to brach on
    # so we will announced all nodes are split!! and will solve them and bye.
    ...
    if isinstance(domains, ShallowFirstBatchedDomainList) and domains.use_bfs:
        net.build_history_and_set_bounds(
            d, split, impl_params=impl_params, mode="breadth"
        )
    else:
        net.build_history_and_set_bounds(
            d, split, impl_params=impl_params, mode="depth"
        )
    ...
    #
    # ret['lower_bounds']['/input-3'] -> [2, 50]
    #

    # ZD, If all nodes have been split (i.e., no more branching is possible),
    # but the instance is still not verified, this flag allows the verifier to
    # attempt LP (Linear Programming) to try and verify the instance.
    # It’s a last-resort verification method in cases where standard bound propagation
    ...
    # [MAX] the fix interm bounds is only true when cuts are enabled
    print("Checking feasibility?", not fix_interm_bounds)
...
def act_split_round(
    ...
    if biccos_enable and not bab_args["cut"]["cplex_cuts"]:
        biccos_update_general_beta(net, d)
    ...
    # [OVAL-GNN]
    # if torch.isinf(ret[0]):
    # print("LP in OVAL-GNN heuristic shows infeasibility.")
    # print(f"{stats.visited} domains visited")
    # return ret[0]
...
def multi_tree_bab(
    net, domains, batch, stop_criterion, biccos_args, impl_params, stats, start_time
):
    """
    ...
    while num_domains > 0 and total_round < max_iter_shallow:
        ...
        num_domains = len(domains)
        print(f"[ZRL]Cumulative time: {time.time() - start_time}\n")
    ...
    # ZD, “infeasibility” refers to whether a subdomain can be ruled out
    # because it cannot contain any counterexample — meaning,
    # it violates the constraints and thus cannot be part of a valid solution.
...
def general_bab(
    ...
    domain,
    x,
    refined_lower_bounds=None,
    refined_upper_bounds=None,
    ...
    biccos_args = bab_args["cut"]["biccos"]
    multi_tree_branching_enabled = (
        cut_enabled
        and biccos_args["enabled"]
        and biccos_args["multi_tree_branching"]["enabled"]
    )
    max_iterations = max_iterations or bab_args["max_iterations"]

    input_relu_splitter = (
        InputReluSplitter() if branch_args["branching_input_and_activation"] else None
    )
    ...
    if refined_lower_bounds is None or refined_upper_bounds is None:
        ...
        if activation_opt_params is not None:
            del activation_opt_params
    ...
    if cut_enabled:
        net.set_cuts(
            model_incomplete.A_saved, x, ret["lower_bounds"], ret["upper_bounds"]
        )
    ...
    # [MAX] added to be able to log more things, e.g. unstable neurons at start.
    arguments.Globals["out"]["initial_unstable_neurons"] = tot_ambi_nodes

    # [MAX] add to check if proceed to bab, or if we need to do more preprocessing with the split.
    # tot_nodes = 0
    # for _, masks in updated_mask.items():
    #     tot_nodes += int(sum([sum(mask.size()) for mask in masks]))

    # min_threshold = arguments.Config["general"]["min_stable_neurons_ratio_threshold"]
    # stable_ratio = 1 - (tot_ambi_nodes / tot_nodes)

    # arguments.Globals["out"]["stable_ratio"] = stable_ratio

    # if stable_ratio < min_threshold and not np.isclose(min_threshold, 0.0):
    #     print("Exit. Too many unstable nodes.")
    #     # stable ratio should be higher than threshold to continue, so if the ratio is lower, we exit.
    ...
    # ZD, checking if we are in testing with traing and testing of prepared data
    prepared_buffer = arguments.Config["bab"]["branching"]["rl"]["training"][
        "preparedBuffer"
    ]
    testing_prepared_buffer = arguments.Config["bab"]["branching"]["rl"]["training"][
    ...
    if testing_prepared_buffer == True:
        print(type(net))
        return net

    print(f"[ZRL][general_bab] Entering BnB loop: {num_domains} domains, rhs = {rhs}")
    ...
    while num_domains > 0:
        ...
        if input_relu_splitter and input_relu_splitter.split_condition(total_round - 1):
            print("[ZRL] [general_bab] input_split_on_relu_domains")
            ...
            # print(f"[ZRL] [general_bab] act_split_round")
        ...
        if isinstance(global_lb, torch.Tensor):
            # NOTE [ZRL] Why reduce the global lower bound? Let's keep it
            myglb = global_lb.detach().clone().cpu()
        ...
        if stats.all_node_split:
            ...
            print("[ZRL] Maximum number of visited domains has reached.")
            result = "unknown"
        elif max_iterations != -1 and total_round == max_iterations:
            print("[ZRL] Maximum number of BnB iterations has been reached.")
            result = "unknown"
        # ZD, commented this for experiment using steps
        elif time.time() - start_time > timeout:
            print("[ZRL] Time out!!!!!!!!")
            result = "timeout"
        # [OVAL-GNN]
        elif torch.isinf(torch.tensor(global_lb)):
            result = "infeasible"
    ...
    print(
        f"[ZRL] -> domains visited : {stats.visited + 1} | result: {result} | steps: {total_round}"
    )

    arguments.Globals["out"]["total_round"] = total_round
    arguments.Globals["out"]["domains_visited"] = stats.visited
