...
def set_cuts(self: 'LiRPANet', A, x, lower_bounds, upper_bounds,
    ...
    if (arguments.Config["bab"]["cut"]["cplex_cuts"] and self.mip_building_proc is None) or arguments.Config["bab"]["cut"]['biccos']["enabled"]:
...
def set_cut_params(self: 'LiRPANet', batch_size, batch_base, split_history):
    ...
    if split_history is not None and not arguments.Config["bab"]["cut"]['biccos']["enabled"]:
