...
class ConfigHandler:
    ...
    def add_common_options(self):
        ...
        # NOTE added by Max to have easier access to output stats.
        self.add_argument("--output_additional_stats", action='store_true',
                          help="Add time and decision stats in output file for single_vnnlib mode (utils:97).",
                          hierarchy=h + ["output_additional_stats"])

        # NOTE added by Max to keep track of instance id when providing .onnx and .vnnlib files.
        self.add_argument("--instance_id", type=int, default=0,
                    help="A way to provide the current instance id.",
                    hierarchy=h + ["instance_id"])

        # NOTE added by Max to configure output path for dataset generation
        self.add_argument("--dataset_output_dir", default="dataset",
                          help="A dataset path for storing the generated data use the generate_dataset heuristic.",
                          hierarchy=h + ["dataset_output_dir"])


        # NOTE added by Max as a simple check for preprocessing input splits before going into bab.
        self.add_argument("--min_stable_neurons_ratio_threshold", type=float, default=0.0, 
                          help="This flag checks if the ratio of stable neurons is at least a some perecentage, e.g., 70%% of nodes must be stable before we proceed with bab, otherwise, exit. This is useful as a basic heursitic for preprocessing input splits. If set to 0.0 disables the check.",
                          hierarchy=h + ["min_stable_neurons_ratio_threshold"])
        ...
        self.add_argument('--onnx_quirks', type=str, default=None,
                  help=("Load onnx model with quirks to workaround onnx model issue. "
                      "This string will be passed to onnx2pytorch as the 'quirks' argument, and it is typically a "
                      "literal of a python dict, e.g., \"{'reshape': {'fix_batch_size': True}}\"."),
        ...
        self.add_argument('--mip_refine_remove_neurons', action='store_false', dest='remove_unstable_neurons',
        ...
        self.add_argument("--gnn_path", type=str, default="",
                          help="Path to the gnn heuristic model.",
                          hierarchy=h + ["gnn_path"])
        
        h = ["bab", "branching", "neural_heuristic"]

        self.add_argument("--model_path", type=str, default="",
                          help="Path to the neural heuristic model.",
                          hierarchy=h + ["model_path"])
     

        h = ["bab", "branching", "rl"]
        
        self.add_argument("--rlExperiment", type=str, default="general",
                        help="Custom RL branching experiment model name.",
                        hierarchy=h + ["experiment"])
        
        self.add_argument("--rlEval", type=bool, default=False,
        ...
        self.add_argument(
            "--rlExtras",
            type=str,
            nargs="*",  # or "*" if optional
            default=[],
            help="List of extra input features to include in the state representation.",
            hierarchy=h + ["extras"]
        )

        h = ["bab", "branching", "rl", "logging"]

        self.add_argument("--rlLogColor", type=bool, default=True,
                        help="Color-enabled logging.",
                        hierarchy=h + ["color"])
        
        self.add_argument("--rlLogDebug", type=bool, default=True,
                        help="Debugging mode.",
                        hierarchy=h + ["debug"])
        
        self.add_argument("--rlLogFile", type=str, default="rl-experiment.log",
                        help="Log to file.",
                        hierarchy=h + ["file"])
        
        h = ["bab", "branching", "rl", "pnet"]

        self.add_argument("--rlPolicyNetGrH", type=int, default=64,
                        help="Hidden dimension for graph encoder.",
                        hierarchy=h + ["gH"])
        
        self.add_argument("--rlPolicyNetTrH", type=int, default=64,
                        help="Hidden dimension for transformer encoder.",
                        hierarchy=h + ["tH"])
        
        self.add_argument("--rlPolicyNetNHeads", type=int, default=64,
                        help="Number of transformer heads.",
                        hierarchy=h + ["nheads"])
        
        self.add_argument("--rlPolicyNetNLayers", type=int, default=64,
                        help="Number of transformer layers.",
                        hierarchy=h + ["nlayers"])
        
        self.add_argument("--rlUseDynamicGraph", type=bool, default=True,
                        help="Use dynamic graph.",
                        hierarchy=h + ["useDynamicGraph"])

        self.add_argument("--rlBidirectional", type=bool, default=False,
                  help="Use bidirectional edges in the graph GNN (True) or unidirectional (False).",
                  hierarchy=h + ["bidirectional"])
        
        self.add_argument("--rlGAT", type=bool, default=True,
                        help="Use GATConv (instead of GConv) for graph encoding.",
                        hierarchy=h + ["GAT"])
        
        h = ["bab", "branching", "rl", "training"]

        self.add_argument("--rlGamma", type=float, default=0.99,
                        help="Discount factor gamma for RL.",
                        hierarchy=h + ["gamma"])
        
        self.add_argument("--rlLearningRate", type=float, default=1e-3,
                        help="Learning rate for RL-based branching.",
                        hierarchy=h + ["lr"])
        
        self.add_argument("--rlBatchSize", type=int, default=8,
                        help="Training batch size for RL.",
                        hierarchy=h + ["batchSize"])
        
        self.add_argument("--rlTrainingLoop", type=int, default=8,
                        help="Training loop.",
                        hierarchy=h + ["loop"])

        self.add_argument("--rlTotalEpochs", type=int, default=10,
                  help="Total number of RL training epochs (used to repeat over training dataset).",
                  hierarchy=h + ["totalEpochs"])
        
        self.add_argument("--rlUpdatePolicyFrequency", type=int, default=10,
                        help="Number of samples after which to train policy network.",
                        hierarchy=h + ["updatePolicyFrequency"])
        
        self.add_argument("--rlUpdateTargetFrequency", type=int, default=10,
                        help="Number of samples after which to update target model.",
                        hierarchy=h + ["updateTargetFrequency"])
        
        self.add_argument("--rlBuffer", type=str, default="buffer.pkl",
                        help="Buffer file.",
                        hierarchy=h + ["buffer"])

        self.add_argument("--rlPreparedBuffer", type=str, default='False',
                        help="Path to a prepared buffer file.",
                        hierarchy=h + ["preparedBuffer"])

        self.add_argument("--rlTestingPreparedBuffer", type=str, default=False,
                        help="Testing on prepared buffer.",
                        hierarchy=h + ["TestingPreparedBuffer"])

        self.add_argument("--rlScriptTraining",
                        action="store_true",
                        help="Run in script mode (use prepared buffer). If not set, runs pipeline mode.",
                        hierarchy=h + ["scriptTraining"])
        
        self.add_argument("--rlPolicyNet", type=str, default="policy-checkpoint.pt",
                        help="Checkpoint file for policy-net.",
                        hierarchy=h + ["policynet"])
        
        self.add_argument("--rlTargetNet", type=str, default="target-checkpoint.pt",
                        help="Checkpoint file for target-net.",
                        hierarchy=h + ["targetnet"])

        self.add_argument("--rlDatasetShuffle", type=bool, default=True,
                 help="Shuffle whole dataset instances before splitting.",
                 hierarchy=h + ["datasetShuffle"])

        self.add_argument("--rlTargetData", type=str, choices=["all", "bnb", "safe"], default="all",
            help="Targeted data selection for RL: 'all' instances, 'bnb' instances or only 'safe' instances.",
            hierarchy=h + ["targetedData"]
        )

        self.add_argument("--rlSplitRatio", type=float, default=0.8,
                        help="Proportion of data used for training (e.g., 0.8 means 80 train / 20 test).",
                        hierarchy=h + ["splitRatio"])

        self.add_argument("--rlMaxInstances", type=int, default=-1,
                help="Maximum number of RL training instances per experiment.",
                hierarchy=h + ["maxInstances"])  

        self.add_argument("--rlTrainingMode", type=str, default="full",
                  choices=["full", "imitation", "qlearn"],
                  help="Training mode: 'imitation' for expert-only, 'qlearn' for RL-only, 'full' for both.",
                  hierarchy=h + ["trainingMode"])             

        h = ["bab", "branching", "rl", "training", "reward"]

        self.add_argument(
            "--rlRewardNormalizationMethod",
            type=str,
            default=None,
            help="Reward normalization method (e.g., 'sum', 'zscore', 'minmax', or None).",
            hierarchy=h + ["norm"]
        )

        self.add_argument("--rlRewardBonus", type=float, default=10.0,
                          help='Reward bonus for successful episodes.',
                          hierarchy=h + ['bonus'])
        
        self.add_argument("--rlRewardPenalty", type=float, default=-1.0,
                          help='Penalty for unsuccessful episodes.',
                          hierarchy=h + ['penalty'])
        
        h = ["bab", "branching", "rl", "egreedy"]

        self.add_argument("--rlEgreedyEpsilon", type=float, default=0.1,
                        help="Epsilon for e-greedy RL exploration.",
                        hierarchy=h + ["epsilon"])
        
        self.add_argument("--rlEgreedyEpsilonDecay", type=float, default=0.1,
                        help="Epsilon decay for e-greedy RL exploration.",
                        hierarchy=h + ["epsilonDecay"])
        
        self.add_argument("--rlEgreedyMinEpsilon", type=float, default=0.1,
                        help="Minimum epsilon for e-greedy RL exploration.",
                        hierarchy=h + ["minEpsilon"])

        self.add_argument("--rlEgreedyPath", type=str, default="epsilon.pt",
                        help="Path to store and load epsilon value.",
                        hierarchy=h + ["ePath"])
        
        self.add_argument("--rlEgreedyEnabled", type=bool, default=True,
                        help="Enable e-greedy RL exploration.",
                        hierarchy=h + ["enabled"])
        
        h = ["bab", "branching", "rl", "expert"]

        self.add_argument("--rlUseExpertUntilEpisode", type=float, default=0.1,
                        help="Epsilon for e-greedy RL exploration.",
                        hierarchy=h + ["useExpertUntilEpisode"])
        
        self.add_argument("--rlExpertEnabled", type=bool, default=True,
                        help="Enable immitation for RL training.",
                        hierarchy=h + ["enabled"])
        
        self.add_argument("--rlLearnToImitateExpertEpochs", type=int, default=10,
                          help='Penalty for unsuccessful episodes.',
                          hierarchy=h + ['learnToImitateExpertEpochs'])
        ...
        self.add_argument("--nonlinear_split_relu_only",
                          action="store_true",
                          help="BBPS for ReLU only.", hierarchy=h + ['relu_only'])
        ...
        ### preprocessor-hint: private-section-start
        self.add_argument("--branching_point_node", type=str, default='',
                          help='The node type using pre-computed branching points.',
                          hierarchy=h + ['branching_point_node'])
        self.add_argument("--branching_point_db", type=str, nargs='+', default=[],
                          help='Path to pre-computed branching points.',
                          hierarchy=h + ['branching_point_db'])
        ### preprocessor-hint: private-section-end
...
Globals = ReadOnlyDict({
    ...
    "out": {"idx": None, "pred": None, "attack_margin": None, "pred_adv": None,
            ...
            "time": None, "neurons_visited": None, "initial_unstable_neurons": 0, 
            "stable_ratio": None}})
