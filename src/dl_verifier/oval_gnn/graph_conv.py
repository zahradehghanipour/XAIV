import torch as th
from onnx2pytorch.operations.flatten import Flatten
    ...
    def forward(
        ...
        for i in range(self.T):
            ...
            if i == 0:
                inp = th.cat(
                    [
                        lower_bounds_all[0].view(-1).unsqueeze(-1),
                        primal_inputs.view(-1).unsqueeze(-1),
                        upper_bounds_all[0].view(-1).unsqueeze(-1),
                    ],
                    1,
                )
            ...
            for layer_idx, layer in enumerate(layers["fixed_layers"]):
                ...
                if type(layer) is nn.Conv2d:
                    ...
                    nb_embeddings_temp = th.cat(
                        [
                            nb_embeddings_temp[i * p : (1 + i) * p]
                            for i in range(batch_size)
                        ],
                        1,
                    )
                    nb_embeddings_temp = th.t(nb_embeddings_temp)
                    pre_layer_bias = layer.bias.unsqueeze(1).expand(
                        out_features[1], out_features[2] * out_features[3]
                    )
                    ...
                    ratio_0, ratio_1, beta, ambi_mask = compute_ratio(
                        layer_lower_pre, layer_upper_pre
                    )
                    # import pdb; pdb.set_trace()

                    # measure relaxation
                    layer_n_bounds = th.cat(
                        [
                            beta.unsqueeze(-1),
                            layer_lower_pre.unsqueeze(-1),
                            layer_upper_pre.unsqueeze(-1),
                            dual_vars[relu_count_idx][:, 1].unsqueeze(-1)
                            - dual_vars[relu_count_idx][:, 2].unsqueeze(-1),
                            primals[layer_idx - 1].unsqueeze(-1),
                            primals[layer_idx].unsqueeze(-1),
                            pre_layer_bias.unsqueeze(-1),
                        ],
                        1,
                    )
                    ...
                    nb_embeddings_input = th.cat(
                        [
                            nb_embeddings_temp * ratio_0.unsqueeze(-1),
                            nb_embeddings_temp * ratio_1.unsqueeze(-1),
                        ],
                        1,
                    )
                    layer_nb_embeddings = self.fc3_2(
                        F.relu(self.fc3(nb_embeddings_input))
                    )
                    # import pdb; pdb.set_trace()
                    ...
                    out_features = [-1] + th.tensor(
                        lower_bounds_all[idx + 1].size()
                    ).tolist()
                    pass
                else:
            ...
            # forward pass
            nb_embeddings_temp = [
                layers["prop_layers"][i].weight @ mu[relu_count_idx][i]
                for i in range(batch_size)
            ]
            ...
            ratio = lower_bounds_all[0].new_full(
                lower_bounds_all[-1].size(), fill_value=1.0
            )
            ratio = [
                th.t(layers["prop_layers"][i].weight) @ ratio[i]
                for i in range(batch_size)
            ]
            ...
            for layer_idx, layer in reversed(list(enumerate(layers["fixed_layers"]))):
                ...
                if type(layer) is nn.Conv2d:
                    ratio = F.conv_transpose2d(
                        ratio, layer.weight, stride=layer.stride, padding=layer.padding
                    )
                    ...
                    if type(layers["fixed_layers"][layer_idx - 1]) is nn.Conv2d:
                        out_features = lower_bounds_all[idx].size()
                        pre_layer_bias = pre_layer_bias.unsqueeze(1).expand(
                            out_features[1], out_features[2] * out_features[3]
                        )
                        pre_layer_bias = pre_layer_bias.reshape(-1)
                    ...
                    layer_relax_s1 = self.bc1_2(
                        F.relu(self.bc1_1(F.relu(self.bc1(layer_n_bounds))))
                    )
                    # layer_relax_s1 = self.bc1_1(F.relu(self.bc1(layer_n_bounds)))
                    layer_relax_s2_input = th.cat(
                        [
                            layer_relax_s1,
                            layer_relax_s1
                            * (-dual_vars[relu_count_idx - 2][:, 2].unsqueeze(-1)),
                    ...
                    if type(next_layer) is nn.Conv2d:
                        ...
                        mu_inp = th.cat([i for i in mu[relu_count_idx]], 1)
                        mu_inp = th.t(mu_inp).reshape(out_features)
                        nb_embeddings_pre = F.conv_transpose2d(
                            mu_inp,
                            nor_weight,
                            bias=None,
                        ...
                        freq_inp = th.ones([1, 1, w_inp, h_inp])
                        freq_weight = th.ones([1, 1, w_wgt, h_wgt])
                        freq = F.conv_transpose2d(
                            freq_inp,
                            freq_weight,
                            bias=None,
                            stride=next_layer.stride,
                        ...
                        nb_embeddings = th.cat(
                            [
                                nb_embeddings[i * p : (1 + i) * p]
                                for i in range(batch_size)
                            ],
                            1,
                        )
                    ...
                    elif next_layer == "Linear":
                        nb_embeddings = th.t(nor_weight).matmul(mu[relu_count_idx])
                        nb_embeddings = th.cat([i for i in nb_embeddings], 0)

                    elif next_layer == "prop":
                        # FIX the nans start appearing here! mu contains nans.
                        nb_embeddings = [
                            th.t(nor_weight[i]) @ mu[relu_count_idx][i]
                            for i in range(batch_size)
                        ]
                    ...
                    layer_embeddings_input = th.cat(
                        [nb_embeddings_0, nb_embeddings_1], dim=1
                    )
                    layer_nb_embeddings = self.bc3_1(
                        F.relu(self.bc3(layer_embeddings_input))
                    )
                    ...
                    layer_mu_new = layer_mu_new * (
                        (ratio_0 != 0).view(-1)
                    ).float().unsqueeze(-1)

                    mu[relu_count_idx - 1] = layer_mu_new.reshape(
                        mu[relu_count_idx - 1].size()
                    )
                    relu_count_idx -= 1
                ...
                if layer_idx == 0:
                    ...
                    mu[relu_count_idx - 1] = domain_mu_new.reshape(
                        mu[relu_count_idx - 1].size()
                    )
...
def init_mu(lower_bounds_all, p):
    ...
    for i in lower_bounds_all:
        ...
        mus_current_layer = lower_bounds_all[0].new_full(
            (batch_size, required_size[0], p), fill_value=0.0
        )
