    ...
    def summarize_results(self, verified_status, index):
        ...
        if self.run_mode == 'single_vnnlib':
            ...
            with open(self.save_path, 'w') as file:
                file.write(verified_status) # result
           
                if arguments.Config['general']['output_additional_stats']:
                    # prints in order: result, lb_minus_rhs, domains_visited, bab_time, all_time
                    file.write(",")
                    if len(self.bab_ret) != 0:
                        for i in range(1,4):
                            file.write(str(self.bab_ret[0][i])), # 1: lb_minus_rhs, 2: domains_visited, 3: bab_time
                            file.write(',')
                    file.write(str(time.time() - self.start_time)) # all_time
            ...
            # ZD, since I am using steps I don't need this
