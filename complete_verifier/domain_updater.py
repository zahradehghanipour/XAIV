...
class DomainUpdater:
    ...
    def set_branched_bounds(self, d, split, mode='depth', branching_heuristic='kfsb'):
        """
        d: Domains
        ...
        # AK/ZRL
        if branching_heuristic == 'RL':
            if 'uids' in d and 'pids' in d:
                # NOTE
                # We do not support batches!
                #
                assert len(d['uids']) == 1 and self.num_copy == 2
                d['pids'] = [uid for uid in d['uids']]
                d['pids'] = d['pids'] * self.num_copy
...
class DomainUpdaterSimple(DomainUpdater):
    ...
    def _append_history(self, idx, node, this_loc, this_sign, this_points):
        ...
        if len(self.new_history[idx][node][0]) == 0:
            ...
            if self.new_history[idx][node][3] is not None and self.new_history[idx][node][3].numel() > 0:
                score[:shape] = self.new_history[idx][node][3]

            # Ensure the source tensor has enough elements to be assigned to depth
            if self.new_history[idx][node][4] is not None:
