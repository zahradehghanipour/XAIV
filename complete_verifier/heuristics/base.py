...
class NeuronBranchingHeuristic():
    ...
    def get_branching_decisions(self, domains, split_depth=1, **kwargs):
        ...
        for idx, score in self.layer_iterator(layer_scores):
            # print(f"[DBG] Gen output for layer {idx} in get_branching_decisions")
            # split_mask = 1 => a neuron can be split.
            # NOTE ZRL: We avoid zero scores
            score += split_masks[idx] * 1e-10
            # print("score shape:", score.shape, "score max :", score.max(), "score min :", score.min())
    ...
    def find_topk_scores(self, layer_scores, split_masks, k, return_scores=False):
        ...
        for idx, layer in enumerate(self.net.split_nodes):
            if layer.name not in layer_scores:
                continue
            # print(f"[ZRL] [find_topk_scores] id {idx:02d} layer {layer.name}")
            scores = layer_scores[layer.name]
            mask = split_masks[layer.name]
            if torch.isnan(scores).any():
                print(f"[ZRL] [find_topk_scores] NaN detected in `scores` for layer {layer.name}")
            if torch.isnan(mask).any():
                print(f"[ZRL] [find_topk_scores] NaN detected in `mask` for layer {layer.name}")
            if not ((mask == 0) | (mask == 1)).all():
                warn(f"[ZRL] [find_topk_scores] Mask not binary in layer {layer.name}: unique = {mask.unique()}")
            ...
            if this_layer_k > 0:
                ...
                if torch.isnan(tmp).any():
                    print(f"[ZRL] [find_topk_scores] NaN detected in `scores * mask` for layer {layer.name}")
        ...
        # print(f"[ZRL] [find_topk_scores] max_valid_scores = {max_valid_scores}")
        # print(f"[ZRL] [find_topk_scores] max_k = {max_k}")

        ret = (topk_neuron_layers[:, :max_k], topk_neuron_indices[:, :max_k])
        # print("[ZRL] [find_topk_scores] return nodes:", ret)
        # print("[ZRL] [find_topk_scores] return scores:", topk_scores[:, :max_k])
        if return_scores:
    ...
    def format_decisions(self, layers, indices, points=None):
        ...
        # print("[ZRL] [format_decisions]", stacked_decisions, points, split_depth)
