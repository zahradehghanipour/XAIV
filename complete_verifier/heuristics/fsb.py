...
class FsbBranching(BabsrBranching):
    ...
    def layer_by_name(model: nn.Module, name: str) -> nn.Module:
        for n, m in model.named_modules():
            if n == name:
                return m
        raise KeyError(f"Layer '{name}' not found. Check model.named_modules().")

    @torch.no_grad()
    def predict_class(model, x):
        out = model(x)
        # choose your target: argmax, or a margin logit, etc.
        return int(out.argmax())

    def xai_scores_from_lb(
        model: nn.Module,
        lb_input: torch.Tensor,           # (1,3,H,W) lower-bound image (already normalized like training)
        layer_names: list[str],           # e.g. ["/input", "/input-8", "/input-12"]
        target: int | None = None,
        n_steps: int = 32,
    ):
        model.eval()
        if target is None:
            target = predict_class(model, lb_input)

        baseline = torch.zeros_like(lb_input)
        results = {}   # layer_name -> 1D tensor of scores (Conv: C*H*W, Linear: F)

        for lname in layer_names:
            layer = layer_by_name(model, lname)  # must match model.named_modules() keys
            lig = LayerIntegratedGradients(lambda x: model(x), layer)
            # attr shape:
            #   Conv:  (1, C, H, W)
            #   Linear:(1, F)
            attr = lig.attribute(inputs=lb_input, baselines=baseline, target=target, n_steps=n_steps)
            A = attr.squeeze(0).abs()
            if isinstance(layer, nn.Conv2d):
                results[lname] = A.reshape(-1)      # flat (C*H*W,)
            elif isinstance(layer, nn.Linear):
                results[lname] = A.reshape(-1)      # (F,)
            else:
                # if it’s neither conv nor linear, just flatten
                results[lname] = A.reshape(-1)
    ...
    def get_branching_decisions(
        ...
        for lb in domains["lower_bounds"].keys():
            t = domains["lower_bounds"][lb]
            print(f"{lb:5s} → {t.shape}")
            layer_names.append(lb)


        print(f"layer_names:{layer_names}")

        # lb_input: take the verifier's lower-bound image for this instance
        # lb_input = domains["lower_bounds"][:1]  # (1,3,32,32) — already in the model's normalized space

        scores, tgt = xai_scores_from_lb(self.net, lower_bounds, layer_names, target=None, n_steps=32)
        ...
        if len(tmp_ret):
            ...
            for b in range(split_depth * batch):
                ...
                # ZD, TODO: I modified this piece of code due to an error encountered in ACAS instance 117. Further investigation is needed.
