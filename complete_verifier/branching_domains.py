...
class BatchedDomainList(AbstractDomainList):
    ...
    def __init__(self, ret, lAs, global_lbs, global_ubs,
        ...
        self.branching_heuristic = arguments.Config['bab']['branching']['method']
        ...
        self.depths = [0] * num
        # AK/ZRL: The unique id, and parent id of each domain
        if self.branching_heuristic == 'RL':
            self.uids = [0] * num
            self.pids = [None] * num
        ...
        if arguments.Config['bab']['tree_traversal'] == 'breadth_first':
            ...
            # AK/ZRL
            if self.branching_heuristic =='RL':
                self.uids = deque(self.uids)
                self.pids = deque(self.pids)
        ...
        # AK/ZRL
        # At this point, at depth 0 we already have a node in `domains`.
        if self.branching_heuristic == 'RL':
            self.__NEXTUID = 1
            self.__uids = []
            self.__pids = []
    
    def _nextid(self, parents):
        children = []
        for pid in parents:
            uid = self.__NEXTUID
            self.__NEXTUID += 1
    ...
    def tree(self):
        return self.__uids, self.__pids
    
    def subdomainssolved(self, bounds, d, infeasibility):
        thresholds = d['thresholds']
        glb = bounds['lower_bounds'][self.final_name]
        batch = glb.size(0)
        device = glb.device
        thresholds = thresholds.to(device, non_blocking=True)
        
        if infeasibility:
            infeasible = check_infeasible_bounds(bounds['lower_bounds'], bounds['upper_bounds'])
        else:
            infeasible = None

        indexer = torch.all(glb <= thresholds[:batch], dim=1)
        if infeasible is not None:
            indexer = torch.logical_and(indexer, torch.logical_not(infeasible))
        return ~indexer
    
    def extractstate(self, bounds, d):
        #
        # What information does `bounds` contain?
        #
        # 'lower_bounds'
        # 'upper_bounds'
        # 'lAs'
        ...
        def __getmasks(lb, ub):
            ...
            for k in lb:
                ...
                for activation, index in self.net.split_activations[k]:
                    mask_ = activation.get_split_mask(lb[k], ub[k], index)
                    mask = mask_ if mask is None else torch.logical_or(mask, mask_)
                if mask is None:
                    mask = torch.ones_like(lb[k], dtype=torch.bool)
                masks[k] = mask
            return masks
        
        def __extractstate(idx):
            ...
            # Compute new mask
            msk = __getmasks(lbs, ubs)

            # Get global lower bounds
            glb = bounds['lower_bounds'][self.final_name][idx]

            # Get thresholds (from solved domain)
        ...
        left, right = __extractstate(0), __extractstate(1)
        return left, right
    ...
    def sort(self):
        ...
        for name in ('histories', 'depths', 'uids', 'pids', 'split_histories',
    ...
    def pick_out(self, batch, device='cpu', impl_params=None):
        ...
        if arguments.Config['bab']['tree_traversal'] == 'breadth_first':
            ...
            # AK/ZRL
            if self.branching_heuristic == 'RL':
                uids = [self.uids.popleft() for _ in range(batch)]
                pids = [self.pids.popleft() for _ in range(batch)]
            #
            self.u -= batch
        else:
            ...
            # AK/ZRL
            if self.branching_heuristic == 'RL':
                uids = self.uids[self.u - batch: self.u]
                pids = self.pids[self.u - batch: self.u]  
            #
            self.u -= batch
            ...
            # AK/ZRL
            if self.branching_heuristic == 'RL':
                self.uids = self.uids[:self.u]
                self.pids = self.pids[:self.u]
    ...
    def add(self, bounds, d, check_infeasibility):
        ...
        if len(indexer) > 0:
            # print(f"___________Add {len(indexer)} domains")
            ...
            # AK/ZRL
            if self.branching_heuristic == 'RL':
                children, parents = self._nextid(selector(d['pids'])) 
                self.uids.extend(children)
                self.pids.extend(parents)
            #
