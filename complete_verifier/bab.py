...
from heuristics import RLNeuronBranching
from heuristics.generate_dataset import GenerateDataFsbBranching
...
from cuts.cut_utils import fetch_cut_from_cplex, clean_net_mps_process, cplex_update_general_beta, biccos_update_general_beta
...
def split_domain(net: LiRPANet, domains, d, batch, impl_params=None, stats=None,
    ...
    if isinstance(branching_heuristic, RLNeuronBranching):
        # AK/ZRL
        #
        indexer = domains.subdomainssolved(ret, d, infeasibility=not fix_interm_bounds)
        # [True, True] means both subdomains are solved
        solved = indexer.all().item()

        # Updated our BnB tree
        branching_heuristic.updateTree(*domains.tree(), d['pids'][0], indexer.sum().item())

        if len(domains) == 0:
            #
            # At this point, we are about to finish the verification, since there are
            # no domains left to solve.
            #
            if not solved:
                raise ValueError("Inconsistent state in RL-based verification")
            
            print("[ZRL] Problem solved (no domains left, both subdomains solved)")
            left, right = domains.extractstate(ret, d)

            branching_heuristic.laststep(left)
...
def act_split_round(domains, net, batch, iter_idx, stats=None, impl_params=None,
    ...
    if d['mask'] is not None:
        ret = split_domain(net, domains, d, batch, impl_params=impl_params,
...
def general_bab(net, domain, x, refined_lower_bounds=None,
    ...
    for _, masks in updated_mask.items():
        tot_nodes += int(sum([int(mask.size(1)) for mask in masks]))
    ...
    prepared_buffer = arguments.Config['bab']['branching']['rl']['training']['preparedBuffer']
    testing_prepared_buffer = arguments.Config['bab']['branching']['rl']['training']['TestingPreparedBuffer']
    ...
    while (num_domains > 0):
        ...
        if input_relu_splitter and input_relu_splitter.split_condition(
                total_round-1):
            print(f"[ZRL] [general_bab] input_split_on_relu_domains")
    ...
    print(f'[ZRL] -> domains visited : {stats.visited + 1} | result: {result} | steps: {total_round}')
    ...
    if isinstance(branching_heuristic, RLNeuronBranching):
        # This is ZRL territory...
        branching_heuristic.finalize(result)

    if isinstance(branching_heuristic, GenerateDataFsbBranching):
        # For generating data, save the buffer.
        branching_heuristic.save_buffer()
