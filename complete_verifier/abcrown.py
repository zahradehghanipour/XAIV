...
class ABCROWN:
    ...
    def building_model_for_prepared_data(self, data_lb, data_ub, c, rhs,
            data=None, targets=None, vnnlib=None, timeout=None,
            time_stamp=0, data_dict=None, lower_bounds=None, upper_bounds=None,
            reference_alphas=None, attack_images=None, cplex_processes=None,
            activation_opt_params=None, reference_lA=None,
    ...
    def complete_verifier(
        ...
        try:
            ...
            for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
                ...
                policy_path = arguments.Config['bab']['branching']['rl']['training']['policynet']
                ...
                if not prepared_buffer == 'False': 
                    ...
                    script_training = arguments.Config["bab"]["branching"]["rl"]["training"]["scriptTraining"]
                    if script_training:
                        return net
                    ...
                    n_steps = arguments.Config['bab']['branching']['rl']['training']['maxInstances']
                    
                    print(f"[ZRL]Loading buffer from: {prepared_buffer}")
                    # ZD, TODO, delete this function: split_by_steps
                    train_buffer, test_buffer = branching_heuristic.split_by_steps(f"{exp_dir}/prepared_buffer", prepared_buffer, 0.8, seed, n_steps, True)
                    epochs = arguments.Config['bab']['branching']['rl']['expert']['learnToImitateExpertEpochs']

                    branching_heuristic.imitate_from_prepared_data(train_buffer, policy_path, epochs)
                    return net
                ...
                testing_prepared_buffer = False if testing_prepared_buffer == 'False' else testing_prepared_buffer
                ...
                if testing_prepared_buffer and rl_eval:
                    ...
                    test_buffer = branching_heuristic._buffer.restore(path=f"{exp_dir}/prepared_buffer_test.pkl")
                    print("[ZRL]Test buffer loaded")

                    acc = branching_heuristic.evaluate_imitation_for_prepared_data(test_buffer, policy_path)
                ...
                # MP, added this for a specific heuristic for the preprocess input splitting.
                elif ret == "threshold":
                    return "threshold"
    ...
    def main(self, interm_bounds=None):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            prepared_buffer = arguments.Config['bab']['branching']['rl']['training']['preparedBuffer']
            if not prepared_buffer == 'False':
                print("Imitation using preperd buffer has finished!")
                return verified_status  #this will returns the net

            elif testing_prepared_buffer: 
                print("Testing preperd buffer has finished!")
                return verified_status  #this will returns the net
