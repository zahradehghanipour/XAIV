...
def load_non_global_change_budget_map(stats_csv_path: Path) -> Dict[str, List[Dict]]:
    """
    Load per-image perturbation budgets from an input_change_stats.csv file.
    Keeps only rows where is_global == False.
    """
    budgets: Dict[str, List[Dict]] = defaultdict(list)
    with stats_csv_path.open("r", encoding="utf-8", newline="") as f:
        reader = csv.DictReader(f)
        for row in reader:
            if parse_bool_like(row.get("is_global")):
                continue
            image_key = str(row.get("image", "")).strip()
            if not image_key:
                continue
            try:
                num_changed = int(float(row.get("num_changed", "0")))
            ...
            try:
                eps_val = float(row.get("eps", "nan"))
            except (TypeError, ValueError):
                eps_val = float("nan")

            seg_idx_raw = row.get("segment_index", -1)
            try:
                segment_index = int(seg_idx_raw)
            except (TypeError, ValueError):
                segment_index = -1

            budgets[image_key].append(
                {
                    "num_changed": num_changed,
                    "eps": eps_val,
                    "tag": str(row.get("tag", "csv_non_global")),
...
def process_single_image(
    ...
    non_global_change_budgets: Optional[Dict[str, List[Dict]]] = None,
    ...
    # Optional path: drive perturbation budget from existing CSV rows
    # (is_global=False, using row.num_changed for this image)
    csv_budget_rows = []
    if non_global_change_budgets is not None:
        csv_budget_rows = non_global_change_budgets.get(img_basename, [])

    if csv_budget_rows:
        print(f"[CSV-BUDGET] Using {len(csv_budget_rows)} non-global row(s) for {img_basename}")
        for ridx, budget_row in enumerate(csv_budget_rows):
            tag_raw = str(budget_row.get("tag", "csv_non_global")).strip() or "csv_non_global"
            tag_safe = "".join(ch if ch.isalnum() or ch in {"_", "-"} else "_" for ch in tag_raw)
            segment_index = int(budget_row.get("segment_index", -1))
            num_changed = int(budget_row["num_changed"])

            # num_changed in stats CSV is often input-dimension count (H*W*C).
            # Convert to spatial pixel budget for bounds_for_segment (which perturbs all channels).
            if num_changed > total_pixels:
                if C > 0 and num_changed % C == 0:
                    k_pixels = num_changed // C
                else:
                    k_pixels = int(round(num_changed / max(C, 1)))
            else:
                k_pixels = num_changed
            k_pixels = max(0, min(total_pixels, k_pixels))

            eps_row = budget_row.get("eps", float("nan"))
            eps = float(eps_row) if np.isfinite(eps_row) else float(epsilons[0])
            ...
            vnn_name = (
                f"{img_basename}_nonglobal_random_{tag_safe}_seg{segment_index}"
                f"_n{num_changed}_k{k_pixels}_eps_{eps}.vnnlib"
            )
            ...
            if stats_rows is not None:
                row = report_changed_inputs(
                    ...
                    extra={
                        ...
                        "score": None,
                        "bbox": None,
                        "pattern": "random_from_csv_num_changed",
                        "model": model_tag,
                    },
                )
...
def main():
    ...
    parser.add_argument(
        "--input-change-stats",
        type=str,
        default="benchmarks/cifar100_all/input_change_stats.csv",
        help=(
            "Path to an input_change_stats.csv used to drive random perturbation "
            "budgets from rows where is_global=False."
        ),
    )
    # TODO: delete later
    # import sys
    ...
    non_global_change_budgets: Dict[str, List[Dict]] = {}
    stats_source_path = Path(args.input_change_stats)
    if stats_source_path.exists():
        non_global_change_budgets = load_non_global_change_budget_map(stats_source_path)
        print(
            f"[CSV-BUDGET] Loaded non-global budgets for {len(non_global_change_budgets)} image(s) "
            f"from {stats_source_path}"
        )
    else:
        print(f"[CSV-BUDGET] File not found, skipping CSV-driven budgets: {stats_source_path}")
    ...
    for img_path in images_to_process:
        ...
        rows = process_single_image(
            ...
            non_global_change_budgets=non_global_change_budgets,
        )
