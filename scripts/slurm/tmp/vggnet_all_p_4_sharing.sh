#!/bin/bash
#SBATCH --job-name=vggnet16_all_p_4
...
# ------------------------------------------------------------
# IMPORTANT:
#  - Avoid 'set -u' until AFTER conda is activated and we know HOME exists.
#  - Do NOT load cuda/12.8 module with PyTorch cu121 unless needed.
# ------------------------------------------------------------
set -eo pipefail

# Ensure HOME is set (prevents /etc/profile.d/* scripts from exploding under nounset)
...
# This will surface the real init error if any:
try:
    ...
    print("CUDA init exception:", repr(e))
PY
...
#   - Keep it robust: set HOME, activate conda inside the step
...
# Set to 1 if instances.csv has a header row
HAS_HEADER=0
...
for ID in \$(seq \"\${START_ID}\" \"\${END_ID}\"); do
  ...
  if [[ -z \"\${onnx}\" || -z \"\${vnnlib}\" || -z \"\${timeout}\" ]]; then
    echo \"[rank \${rank}] ERROR: could not read line \${line} (ID \${ID})\" >&2
